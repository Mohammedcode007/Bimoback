...
const targetUserSettings = await User.findById(targetId)
  .select("privacy.allowMessages");

if (!targetUserSettings) {
  throw new Error("Target user not found");
}

// لو قافل الرسائل، اسمح فقط للأصدقاء
if (targetUserSettings.privacy?.allowMessages === false && !relation.isFriend) {
 ...
 /* =====================================================
  ...
  try {
    const isBotChat = await systemBotService.chatContainsOfficialBot(chatId);
    const isSenderBot = await systemBotService.isOfficialBotUser(senderId);

    if (isBotChat && !isSenderBot && type === "text") {
      await systemBotService.handleIncomingMessage({
        chatId,
        senderId,
        content,
        type,
    ...
    console.log("❌ bot auto reply failed:", botError);
  }
