...
import Room, { RoomType, RoomPremiumLevel, RoomBotLanguage } from "../models/Room";
...
const USER_PUBLIC_FIELDS =
  "username atUsername avatar avatarGif coverImage usernameColor messageTextColor isOnline lastSeen role " +
...
const ROOM_BOOST_PRICE = 1500;
const ROOM_BOOST_DURATION_DAYS = 30;
...
class RoomService {
  ...
  async joinRoom(roomId: string, userId: string, password?: string) {
    ...
    let roomName = "";
    let roomBotEnabled = false;
    let welcomeEnabled = false;
    let isEnglishWelcome = false;
    let roomWelcomeMessage = "";
    ...
    const { joined } = await this.withTx(async (session) => {
      ...
      roomBotEnabled = Boolean(room.roomBot?.enabled);
      welcomeEnabled = Boolean(room.roomBot?.welcomeEnabled);

      const botLang = (room.roomBot?.language ?? "ar") as RoomBotLanguage;
      isEnglishWelcome = botLang === "en";

      roomWelcomeMessage = String(room.roomBot?.welcomeMessage || "").trim();
    ...
    if (roomBotEnabled && welcomeEnabled) {
      ...
      const joinedUser = await this.getUserBasic(userId);

      const defaultWelcome = isEnglishWelcome
        ? `Welcome ${joinedUser.username} to ${roomName} 🌟`
        : `أهلاً ${joinedUser.username} في غرفة ${roomName} 🌟`;
      ...
      // 👇 أضف الاسم دائمًا في البداية
      welcomeText = `${username} • ${welcomeText}`;


      const welcomeMsg = await this.system(roomId, welcomeText, "system", {
        systemType: "room_welcome",
        mentions: [userId],
        sender: userId,
      });
  ...
  async inviteToRoom(
    ...
    if (!this.isValidObjectId(targetId)) throw new Error("Invalid targetId");
    if (String(actorId) === String(targetId)) throw new Error("Cannot invite yourself");

    const room = await Room.findById(roomId).select(
      "name avatar type isLocked creator owners admins members activeUsers blockeds"
    );
    if (!room) throw new Error("Room not found");

    this.ensureArrays(room);

    // لازم الداعي يكون له علاقة بالغرفة
    const actorRole = this.getRole(room, actorId);
    const actorInside = this.isInside(room, actorId);

    if (!actorInside && actorRole === "none") {
      throw new Error("Not allowed");
    }

    if (this.isBanned(room, actorId)) throw new Error("You are banned from this room");
    if (this.isBanned(room, targetId)) throw new Error("Target user is banned from this room");
    ...
    let chat = await Chat.findOne({
      participants: {
        $all: [new Types.ObjectId(actorId), new Types.ObjectId(targetId)]
      }
    });

    if (!chat) {
      chat = await Chat.create({
        participants: [new Types.ObjectId(actorId), new Types.ObjectId(targetId)],
        unreadCounts: {
          [actorId]: 0,
          [targetId]: 0
        },
        ...
        archivedBy: [],
        lastMessagePreview: "",
        lastMessageType: "room_invite"
      });
    }

    const safeText = String(inviteMessage || "").trim() || `تعالى انضم لغرفة ${room.name}`;
    ...
    await chat.updateOne(
      { _id: chat._id },
      {
        $set: {
          ...
          lastMessagePreview: `🎯 دعوة لغرفة: ${room.name}`,
          lastMessageType: "room_invite",
          updatedAt: new Date()
        },
        $inc: {
        ...
        $pull: {
          deletedFor: {
            $in: [new Types.ObjectId(actorId), new Types.ObjectId(targetId)]
          }
        }
      }
    );
    ...
    io.to(actorId).emit("chat:inbox:update", {
      chat: chatSnap,
      unreadCount: 0
    });

    io.to(`chat:${String(chat._id)}`).emit("chat:new", message);
    ...
    try {
      await notificationService.create({
        ...
        type: "room_invite",
        title: "دعوة إلى غرفة",
        body: `${actor.username} دعاك إلى غرفة ${room.name}`,
        relatedChat: String(chat._id),
        isRead: false,
        isDeleted: false
      });
    } catch (err) {
