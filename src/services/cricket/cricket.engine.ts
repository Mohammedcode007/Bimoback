...
export class CricketEngine {
  ...
  private expireIfTurnTimedOut(game: CricketGame) {
    if (game.status !== "live") return;

    const lastTurnAt = game.lastTurnAt
      ? new Date(game.lastTurnAt).getTime()
      : 0;

    if (!lastTurnAt) {
      game.lastTurnAt = this.now();
      return;
    }

    const diff = Date.now() - lastTurnAt;

    if (diff < CRICKET_TURN_TIMEOUT_MS) return;

    const currentPlayer = this.getCurrentPlayer(game);

    game.status = "cancelled";
    game.endedAt = this.now();
    game.cancelledReason = currentPlayer?.username
      ? `انتهت اللعبة تلقائيًا لأن ${currentPlayer.username} لم يلعب دوره خلال دقيقة`
      : "انتهت اللعبة تلقائيًا لأن اللاعب لم يلعب دوره خلال دقيقة";
  ...
  startGame(game: CricketGame): CricketGame {
    ...
    game.currentTurnUserId = game.innings.strikerUserId || null;
    game.lastTurnAt = this.now();
    game.cancelledReason = undefined;
  ...
  playChosenBall(game: CricketGame, actorUserId: string, choice: number) {
    this.expireIfTurnTimedOut(game);
