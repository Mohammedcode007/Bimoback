...
import { isUserOnlineNow } from "../sockets/presence.socket";
...
async getFriends(userId: string) {
  ...
  const friends = relations.map((relation: any) => {
    ...
    const realOnline = isUserOnlineNow(String(friend._id));
    const visibleOnline = !friend.isInvisible && realOnline;

    return {
      ...friend,
      isOnline: visibleOnline,
      lastSeen: visibleOnline ? null : friend.lastSeen || null,
    };
  });
