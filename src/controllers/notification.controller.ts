...
export const saveDeviceToken = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ مهم جدًا: شيل التوكن من أي مستخدم آخر أولًا
    const pullResult = await User.updateMany(
      { fcmTokens: token },
      { $pull: { fcmTokens: token } }
    );

    console.log("🧹 token removed from old users:", {
      matchedCount: pullResult.matchedCount,
      modifiedCount: pullResult.modifiedCount,
    });

    // ✅ أضف التوكن للمستخدم الحالي فقط
    const addResult = await User.updateOne(
      { _id: userId },
      { $addToSet: { fcmTokens: token } }
    );

    console.log("✅ token saved to current user:", addResult);
...
export const removeDeviceToken = async (req: Request, res: Response) => {
  try {
    ...
    console.log("🗑️ /notifications/device-token remove", {
      userId,
      tokenPrefix: token?.slice(0, 20),
    });
    ...
    console.log("✅ token removed result:", result);
