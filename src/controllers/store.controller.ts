...
function ensureActiveCustomization(u: any) {
  ...
  if (!Array.isArray(u.activeCustomization.badges)) {
    u.activeCustomization.badges = [];
  }

  if (!isVerificationType(u.activeCustomization.verificationType)) {
    u.activeCustomization.verificationType = "none";
  }

  if (typeof u.activeCustomization.avatarFrame !== "string") {
    u.activeCustomization.avatarFrame = "";
  }

  if (typeof u.activeCustomization.messageEffect !== "string") {
    u.activeCustomization.messageEffect = "";
  }

  if (typeof u.activeCustomization.profileEntryAnimation !== "string") {
    u.activeCustomization.profileEntryAnimation = "";
  }

  if (typeof u.activeCustomization.avatarGif !== "string") {
    u.activeCustomization.avatarGif = "";
  }

  if (typeof u.activeCustomization.usernameColor !== "string") {
    u.activeCustomization.usernameColor = "";
  }

  if (typeof u.activeCustomization.messageTextColor !== "string") {
    u.activeCustomization.messageTextColor = "";
  }
}
        ...
        for (const line of purchaseLines) {
          ...
          if (setActive) {
            ...
            switch (si.type) {
              ...
              case "avatarGif":
                user.activeCustomization.avatarGif = si.key;
                break;

              case "usernameColor":
                user.activeCustomization.usernameColor = si.key;
                break;

              case "messageTextColor":
                user.activeCustomization.messageTextColor = si.key;
                break;
